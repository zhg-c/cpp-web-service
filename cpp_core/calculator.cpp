...
PYBIND11_MODULE(libcore, m)
{
	...
	m.def("optimize_jpeg", [](py::bytes image_bytes_py, int quality) {
        ...
        return py::bytes((const char*)result.data(), result.size()); }, "Optimizes a JPEG file's bytes by re-encoding with a target quality.", py::arg("image_bytes"), py::arg("quality") = 80);
}