...
<template>
  ...
  >
    ...
    >
      ...
      <p
        v-if="imageFile"
        style="margin-top: 10px; font-size: 0.9em; color: #28a745"
      >
        已选择: <strong>{{ imageFile.name }}</strong> | 原始大小:
    ...
    >
      ...
      <p>
        原始大小: <strong>{{ formatBytes(originalSize) }}</strong>
      </p>
      <p>
        优化后大小: <strong>{{ formatBytes(optimizedSize) }}</strong>
      </p>
      <p>
        压缩率: <strong style="color: #dc3545">{{ compressionRatio }}</strong>
      </p>
